...
import json
import requests
from drive.utils.s3 import get_file, upload_image_with_b, init_conn
...
def analytic_video_with_velocity_job(name_fvideo, velocity, parent):
    api = PotholeDetectionVelocityAPI()
    doc_video = frappe.get_doc('Drive Entity', name_fvideo)
    try:
        response = get_file(doc_video.path)
        if response is None:
        ...
        for item in detection_results:
            ...
            upload_image_with_b(image_data, save_path)
        ...
        upload_image_with_b(byte_xlsx.getvalue(), save_path_xlsx)
